...
class RoleManagementControllerIT extends AuthIntegrationTestBase {
  ...
  void listAppliesDomainSortByDirection() throws Exception {
    ...
    Role firstByCode = roleRepository.save(Role.create("SORT_AAA_" + suffix, "A", null, now));
    roleRepository.save(Role.create("SORT_ZZZ_" + suffix, "Z", null, now.plusSeconds(1)));
  ...
  void listAppliesRequestedPageAndPageSize() throws Exception {
    Instant now = Instant.now();
    String suffix = UUID.randomUUID().toString().substring(0, 8);
    roleRepository.save(Role.create("PAGE_AAA_" + suffix, "A", null, now));
    Role secondByCode =
        roleRepository.save(Role.create("PAGE_ZZZ_" + suffix, "Z", null, now.plusSeconds(1)));
