...
class ActionLogReadServiceTest {
  ...
  void searchReturnsActionLogsWithRequestedPageMetadata() {
    ...
    when(actionLogPort.count(query)).thenReturn(11L);
    when(actionLogPort.search(query)).thenReturn(List.of(actionLog));

    PageDTO<ActionLog> result = new ActionLogReadService(actionLogPort).search(query);
  ...
  void searchPreservesRequestedMetadataWhenNoActionLogMatches() {
    ...
    when(actionLogPort.count(query)).thenReturn(0L);
    ...
    verify(actionLogPort, never()).search(query);
  }
