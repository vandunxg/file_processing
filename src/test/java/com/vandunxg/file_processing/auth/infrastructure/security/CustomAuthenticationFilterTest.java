...
class CustomAuthenticationFilterTest {
  ...
  @Mock private RequestAuthenticationService requestAuthenticationService;
  ...
  void setUp() {
    filter =
        new CustomAuthenticationFilter(
            requestAuthenticationService, customAuthenticationEntryPoint);
  ...
  @Test
  void refreshesAuthoritiesFromTheApplicationService() throws Exception {
    UUID userId = UUID.randomUUID();
    UUID sessionId = UUID.randomUUID();
    Jwt jwt = jwt(userId, sessionId);
    SecurityContextHolder.getContext()
        .setAuthentication(new UserAuthentication(jwt, List.of(), userId));
    when(requestAuthenticationService.resolve(userId))
  ...
  void skipsResolutionWhenContextDoesNotContainUserAuthentication() throws Exception {
    ...
    verifyNoInteractions(requestAuthenticationService);
    verify(filterChain).doFilter(any(), any());
  }
