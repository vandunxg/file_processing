...
/**
 * Proves that {@link UserRepository#save(User)} uses {@code saveAndFlush} under the hood, so the
 * unique-constraint race on {@code normalized_username} surfaces synchronously to the caller (as
 * {@link DataIntegrityViolationException}) instead of at a later transaction commit. This is what
 * {@code RegisterService} depends on to turn a race into a clean 409 response. Extends {@link
...
class JpaUserRepositoryIT extends AuthIntegrationTestBase {
  ...
  void save_exactlyOneWinsAndTheOtherThrowsDataIntegrityViolation_whenSameNormalizedUsernameRaces()
    ...
    try {
      ...
      assertThat(firstSucceeded ^ secondSucceeded)
          ...
          .isTrue();
      assertThat(userRepository.existsByNormalizedUsername(User.normalize(sharedUsername)))
          .isTrue();
    } finally {
      executor.shutdown();
    }
  ...
  @Test
  void countActiveAdminsCountsUsersWithTheActiveAdminRole() {
    Role adminRole = roleRepository.findByCode("ADMIN").orElseThrow();
    long before = userRepository.countActiveAdmins();
    Instant now = Instant.now();
    User admin =
        User.adminCreate(
            "admin-count-" + System.nanoTime(),
            "admin-count-" + UUID.randomUUID() + "@example.com",
            "Admin Count",
            "{bcrypt}$2a$stubhash",
            Set.of(adminRole),
            true,
    ...
    userRepository.replaceRoles(saved.getId(), Set.of(adminRole.getId()), now);

    assertThat(userRepository.countActiveAdmins()).isEqualTo(before + 1);
  }
